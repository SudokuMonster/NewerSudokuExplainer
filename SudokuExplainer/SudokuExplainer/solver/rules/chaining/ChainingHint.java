...
public abstract class ChainingHint extends IndirectHint implements Rule, HasParentPotentialHint {
    ...
    Collection<Potential> getChain(Potential target) {
        ...
        Set<Potential> done = new HashSet<>();
        Collection<Potential> todo = new ArrayList<>();
        todo.add(target);
        while (!todo.isEmpty()) {
            Collection<Potential> next = new ArrayList<>();
    ...
    private Collection<ChainingHint> getNestedChains() {
        Collection<ChainingHint> result = new ArrayList<>();
        Set<FullChain> processed = new HashSet<>();
        ...
        for (ChainingHint chain : new ArrayList<>(result)) {
                ...
                if (p.nestedChain != null) {
                    ...
                    if (!processed.contains(f)) {
                        ...
                        if (localCount > nestedViewNum) {
                            return new Pair<>(p.nestedChain, nestedViewNum);
                        }
    ...
    /**
     * Get the end of the sub-chain from which the given
    ...
    public Map<Cell, BitSet> getBluePotentials(Grid grid, int viewNum) {
        ...
        if (viewNum >= 0) {
            ...
            Collection<Potential> blues = new LinkedHashSet<>();
    ...
    public Collection<Potential> getRuleParents(Grid initialGrid, Grid currentGrid) {
        Collection<Potential> result = new LinkedHashSet<>();
        /*
    ...
    protected int getAncestorCount(Potential child) {
        Collection<Potential> ancestors = new HashSet<>();
        Collection<Potential> todo = new ArrayList<>();
        todo.add(child);
        while (!todo.isEmpty()) {
            Collection<Potential> next = new ArrayList<>();
    ...
    public String appendNestedChainsDetails(String result) {
        ...
        nested.append("(Note that each Nested Forcing Chain relies on the fact that some" +
                " blue candidates have been excluded by the main" +
                " Forcing Chain): <br><br>\n");
        ...
        for (ChainingHint nestedHint : nestedChains) {
            ...
            for (Potential target : nestedHint.getChainsTargets()) {
                ...
                nested.append("Chain ").append(index).append(": <b>If ")
                        .append(assumption.toWeakString()).append(", then ")
                        .append(target.toStrongString()).append("</b>")
                        .append(" (View ").append(index).append("):<br>\n");
     ...
     * <p>
     * The embarrassing case we want to prevent with this strategy
