...
import SudokuExplainer.solver.rules.directed.NakedSingle;
import SudokuExplainer.units.Cell;
import SudokuExplainer.units.CellType;
 ...
 * @see SudokuExplainer.solver.checks.DoubleSolutionWarning
 */
public class BruteForceAnalysis implements WarningHintProducer {
     ...
     * for <tt>includeSolution</tt> in the constructor, a hint with the solution
     * is produced.
     * </ul>
     */
    ...
    private boolean analyse(Grid grid, boolean isReverse, Random rnd,
        ...
        if (!isFillable(grid)) return false;
        ...
        for (int value0 = startValue; value0 != stopValue; value0 += delta) {
            ...
            assert leastCell != null;
            if (leastCell.hasPotentialValue(value)) {
                grid.copyTo(savePoint);
                leastCell.setValueAndCancel(value, CellType.Modifiable);
